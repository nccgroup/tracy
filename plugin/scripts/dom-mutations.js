...
(function(){
    ...
    mutations.forEach(function(mutation) {
      mutation.addedNodes.forEach(function(node){
          if(node.nodeType == 1){
            clickToFill(node);
            chrome.runtime.sendMessage({'type': 'dom', 'msg': node.outerHTML, "location": document.location}, null);// For now we don't want to use the callback handler
          } else if (node.nodeType == 3) {
            chrome.runtime.sendMessage({'type': 'text', 'msg': node.wholeText,"location": document.location}, null);// For now we don't want to use the callback handler
          }
      });
    });
  });
