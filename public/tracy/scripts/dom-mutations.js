(() => {
  const addedNodeHandler = (donePromise, parentNode, addedNodes, i) => {
    ...
    // Ignore HTML generated by Tracy.
    if (node.id && node.id === String.TAG_MENU) {
      return nextStep(donePromise, parentNode, addedNodes, i + 1);
    }
    ...
    nextStep(donePromise, node, addedNodes, i + 1);
  };
      ...
      const send = (chunk) => {
        window.requestAnimationFrame(() => {
          try {
            ...
            tracyRPC.bulkJobs(document.location.href, copy.splice(0, chunk));
          } catch (e) {
